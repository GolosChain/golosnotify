...
NTYPES = {
  'total'           : 0, 
  'feed'            : 1, # comment_reply
  'reward'          : 2, # not supported yet?
  'send'            : 3, # transfer
  'mention'         : 4, # comment -> mention
  'follow'          : 5, #
  'vote'            : 6, # not supported yet?
  'comment_reply'   : 7, #
  'post_reply'      : 8, # not supported yet?
  'account_update'  : 9, #
  'message'         : 10, # not supported yet?
  'receive'         : 11 # transfer
}
...
def processMentions(author_account, text, op):
    ...
    if op['parent_author']:
        what = 'comment'
        url = '%s/@%s/%s#@%s/%s' % (os.environ['STEEMIT_WEBCLIENT_ADDRESS'], op['parent_author'], op['parent_permlink'], op['author'], op['permlink'])
    else:
        what = 'post'
